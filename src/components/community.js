...
function Community() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5002/communities`)
      .then((response) => {
  ...
  const fetchMessages = (communityId) => {
    axios
      .get(`http://localhost:5002/community/messages/${communityId}`)
      .then((response) => {
        if (response.status === 200) {
  ...
  const addComment = () => {
    ...
    axios
      .post(
        "http://localhost:5002/addmessage",
        {
          content: newComment,
  ...
  const deleteComment = (commentId) => {
    axios
      .delete(`http://localhost:5002/addmessage/${commentId}`, {
